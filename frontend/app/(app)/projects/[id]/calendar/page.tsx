...
export default function CalendarPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const tasksWithDates = (Array.isArray(res) ? res : []).filter((task: CalendarTask) => task.due_date || task.start_date);
  ...
  const weeks = useMemo(() => {
    ...
    for (let weekIndex = 0; weekIndex < calendarDays.length; weekIndex += 7) {
      ...
      const overlappingTasks = normalizedTasks.filter((task) => task.rangeStart.getTime() <= weekEnd.getTime() && task.rangeEnd.getTime() >= weekStart.getTime());
      ...
      const segments = overlappingTasks
        .map((task) => {
          ...
          const startCol = diffInDays(weekStart, segmentStart) + 1;
          const endCol = diffInDays(weekStart, segmentEnd) + 1;
          ...
          <h2 className="text-xl sm:text-2xl font-bold text-stone-900 flex items-center gap-2 sm:gap-3">
            ...
            <div className="group relative z-30">
              ...
              <div className="absolute left-0 sm:left-1/2 sm:-translate-x-1/2 top-full mt-2 w-64 sm:w-72 bg-stone-800 text-white text-xs p-3 rounded-lg shadow-xl opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-[80]">
                Vue mensuelle en plages continues. Une tache ou fonctionnalite apparait sous forme de barre sur sa periode, sans repetition dans chaque jour.
              ...
              <div key={weekIndex} className="border-b border-stone-200 last:border-b-0">
                <div className="grid grid-cols-7 gap-[1px] bg-stone-100">
                  {week.days.map((dayInfo, dayIndex) => {
                    ...
                    return (
                      <div
                        key={dayIndex}
                        className={`min-h-[92px] bg-white p-2 transition-colors hover:bg-stone-50/50 ${!dayInfo.currentMonth ? 'opacity-50 bg-stone-50/30' : ''}`}
                      >
                ...
                <div className="relative bg-white px-1 py-2">
                  {week.segments.length === 0 ? (
                    ...
                    <div
                      className="relative grid grid-cols-7 gap-[1px]"
                      style={{ height: `${Math.max(week.laneCount, 1) * 34}px` }}
                    >
                      {week.segments.map((segment) => {
                        ...
                        return (
                          <button
                            ...
                            className={`absolute flex items-center gap-2 rounded-xl border px-3 py-1.5 text-left text-xs font-medium shadow-sm transition-all ${colorClass} ${isDone ? 'opacity-70 line-through' : ''}`}
                            style={{
              ...
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <span className={`px-2 py-0.5 rounded text-[10px] uppercase font-bold tracking-wider ${
                    ...
                    {selectedTask.parent_id == null ? 'Fonctionnalite' : selectedTask.status === 'done' ? 'Termine' : selectedTask.status === 'in_progress' ? 'En cours' : 'A faire'}
                  </span>
                  {selectedTask.phase && (
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4 pt-4 border-t border-stone-100">
                <div>
                  ...
                  <p className={`text-sm font-semibold flex flex-col gap-1.5 ${selectedTask.due_date && new Date(selectedTask.due_date) < new Date() && selectedTask.status !== 'done' ? 'text-red-600' : 'text-stone-800'}`}>
                ...
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-widest text-stone-400 mb-1">Assigne a</p>
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-sm font-semibold text-stone-800 truncate">{selectedTask.assignee_name || 'Non assigne'}</p>
                  </div>
                </div>
              </div>
            </div>
